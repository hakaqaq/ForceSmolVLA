...
def residual_critic_loss(
    ...
    with torch.no_grad():
        ...
        bootstrap = (~batch.terminated & ~batch.truncated).float()
        target = batch.reward.float() + float(gamma) * bootstrap * next_q
...
def residual_actor_loss(
    ...
    *,
    ...
    if valid_count:
        ...
        value = candidate_residual6.sum() * 0.0
    residual = candidate_residual6.square().mean()

    human_count = 0
    human = candidate_residual6.sum() * 0.0

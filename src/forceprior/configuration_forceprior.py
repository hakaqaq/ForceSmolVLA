...
@PreTrainedConfig.register_subclass("force_prior")
@dataclass
class ForcePriorConfig(SmolVLAConfig):
    ...
    def __post_init__(self) -> None:
        ...
        if self.robot_state_feature is None or self.robot_state_feature.shape != (7,):
            raise ValueError("ForcePrior requires observation.state shape (7,)")
        ...
        if (
            ...
            raise ValueError("ForcePrior requires explicit observation.wrench ENV shape (6,)")
        if self.action_feature is None or self.action_feature.shape != (7,):
            raise ValueError("ForcePrior requires action shape (7,)")
        if self.rtc_config is not None:
            raise ValueError("ForcePrior requires rtc_config=None")
        if self.empty_cameras != 0:
            raise ValueError("ForcePrior forbids empty/placeholder cameras")
        if self.adapt_to_pi_aloha or self.use_delta_joint_actions_aloha:
            raise ValueError("ForcePrior forbids inherited Aloha transforms")
...
def load_force_config(
    ...
    *,
    ...
    return ForcePriorConfig(**values)

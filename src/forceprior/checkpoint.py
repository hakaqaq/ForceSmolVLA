...
from .configuration_forceprior import SMOLVLA_CARTESIAN7D, load_force_config
from .configuration_forceprior import OFFLINE_FULL_FINETUNE
from .modeling_forceprior import ForcePriorPolicy
...
EMBEDDED_TRAINING_CONTRACT = "manifests/training_checkpoint_contract.json"
LEGACY_EMBEDDED_TRAINING_CONTRACT = (
    "manifests/training_checkpoint_contract.development.json"
)
DEVELOPMENT_ARTIFACT_TYPES = frozenset(
    {"forceprior_training_checkpoint", "forcesmolvla_training_checkpoint"}
)


def _embedded_training_contract_path(checkpoint_dir: Path) -> Path:
    canonical = checkpoint_dir / EMBEDDED_TRAINING_CONTRACT
    if canonical.is_file():
        return canonical
    return checkpoint_dir / LEGACY_EMBEDDED_TRAINING_CONTRACT
...
def write_development_artifact_manifest(
    ...
    *,
    metadata: dict,
    artifact_type: str = "forceprior_training_checkpoint",
) -> dict:
...
def export_development_actor_checkpoint(
    *,
    ...
    parent_contract = json.loads(
        _embedded_training_contract_path(runtime_parent).read_text(encoding="utf-8")
    )
...
def validate_training_payload_contract(checkpoint_dir: Path) -> dict:
    ...
    try:
        contract = json.loads(
            _embedded_training_contract_path(checkpoint_dir).read_text(encoding="utf-8")
        )
    except (FileNotFoundError, json.JSONDecodeError) as error:
    ...
    if (
        ...
        or contract.get("artifact_type") not in DEVELOPMENT_ARTIFACT_TYPES
...
def prepare_strict_force_config(checkpoint_dir: Path, *, artifact_use: str):
    validate_force_artifact_manifest(checkpoint_dir, artifact_use=artifact_use)
    ...
    if raw.get("type") not in {"force_prior", "force_smolvla"}:
    ...
    from .configuration_forceprior import ForcePriorConfig

    if not isinstance(config, ForcePriorConfig):
...
def load_frozen_base_weights_strict(
    policy: ForcePriorPolicy, checkpoint_file: Path
) -> BaseLoadReport:
...
def load_offline_base_policy(
    ...
    *,
...
) -> tuple[ForcePriorPolicy, BaseLoadReport]:
    ...
    policy = ForcePriorPolicy(config)

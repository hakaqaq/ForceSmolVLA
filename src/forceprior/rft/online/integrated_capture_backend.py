...
from forceprior.rft.online.policy_lineage import InitialGripperAuthority, UPPER_CLOCK_DOMAIN
from forceprior.rft.online.action_representation import (
    quaternion_xyzw_to_rpy_xyz,
)
SHADOW_BACKEND_SCHEMA = "forceprior-stage3-integrated-shadow-backend-v1"
POLICY_EXECUTION_BACKEND_SCHEMA = (
    "forceprior-stage3-integrated-policy-execution-backend-v1"
)
ONLINE_SEMANTICS_VERSION = "forceprior_ack_residual_filter_leash"
...
class _NativeCameraPair:
    ...
    def _run(self) -> None:
        ...
        except Exception as error:  # surfaced by the external observation client
...
class IntegratedCaptureBackend:
    ...
    def _capture_live(
        ...
        if async_learner:
            ...
            profile_value = arguments.get("deployment_profile")
            if not profile_value:
                raise IntegratedCaptureError("SHADOW_DEPLOYMENT_PROFILE_REQUIRED")
            profile_path = Path(str(profile_value)).resolve()

"""Strict model-side input preparation for ForcePrior inference."""
...
from .configuration_forceprior import CAMERA1, CAMERA2
...
def _checkpoint_manifest_path(
    manifests: Path, canonical_name: str, legacy_name: str
) -> Path:
    canonical_path = manifests / canonical_name
    if canonical_path.is_file():
        return canonical_path
    return manifests / legacy_name
...
def load_checkpoint_inference_contract(checkpoint_dir: Path) -> CheckpointInferenceContract:
    ...
    calibration_path = _checkpoint_manifest_path(
        manifests, "calibration_bundle.json", "calibration_bundle.development.json"
    )
    geometry_path = _checkpoint_manifest_path(
        manifests, "wrench_geometry_spec.json", "wrench_geometry_spec.development.json"
    )
    runtime_paths = tuple(
        path
        for path in (
            manifests / "converter_runtime_spec.json",
            *sorted(manifests.glob("converter_runtime_spec.*.json")),
        )
        if path.is_file()
    )

"""Direct FR3 native-stream to ForcePrior LeRobot v3 conversion.
...
def convert_dataset(
    *,
    ...
    if CODEBASE_VERSION != "v3.0":
        raise RuntimeError(f"ForcePrior requires LeRobot codebase v3.0, got {CODEBASE_VERSION}")
    ...
    if session.get("raw_format_version") != RAW_FORMAT or int(session.get("canonical_fps", -1)) != FPS:
        raise ValueError("raw session format/fps does not match ForcePrior v4.1")
    ...
    calibration_path = project_root / "configs" / (
        "calibration_bundle.approved.json" if formal else "calibration_bundle.json"
    )
    geometry_path = project_root / "configs" / (
        "wrench_geometry_spec.approved.json" if formal else "wrench_geometry_spec.json"
    )
    ...
    manifest = {
        ...
        "formal_ready": formal,
        "schema": "forceprior-v4.1-available-sensor-lerobot-v3",
        "repo_id": repo_id,
        ...
        "converter_source": {
            ...
            "implementation": "src/forceprior/raw_to_lerobot_v3.py",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert native FR3 data to the ForcePrior LeRobot v3 contract"
    )
    parser.add_argument(
        "--raw-root",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.development_only:
        ...
        runtime_path = (
            ...
            else project_root / "configs/converter_runtime_spec.json"
        )
